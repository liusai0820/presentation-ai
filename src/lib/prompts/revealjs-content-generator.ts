...
### 布局系统
- \`.two-columns\`: 左右对比、论据支撑
- \`.three-columns\`: 三个并列要点或数据
- \`.grid-2x2\`: 2x2矩阵，四个相关项
...
3. **观点页**：核心观点 + 3-4 个支撑论据（每个有详细说明）
4. **数据页**：3-4 个数据卡片（含数值、标签、解释）或 1 个图表 + 深度解读
5. **对比页**：两栏布局，每栏 3-4 个要点（每个有说明）
6. **案例页**：1-2 个案例，包含背景、数据、结果
7. **总结页**：3-5 个关键结论（每个有简短说明）
...
3. 每个要点必须有详细说明（2-3 句话，40-60 字），不要只列标题
4. 优先使用视觉元素（数据卡片、图表、时间线）增强表现力
5. 保持布局多样性：不同页面使用不同组件和布局
6. 适度分页：只有内容明显过多时才拆分（不要过度分页）
7. 从演讲者角度思考：每页讲透一个核心观点，有足够的论据支撑
8. 严格遵守输出格式：每页以h1或h2开头，页间两个空行分隔

**输出格式示例：**

【封面页】
<h1>海南自贸港：核心战略与发展机遇</h1>
<p>Reveal.js 演示文稿已生成</p>
